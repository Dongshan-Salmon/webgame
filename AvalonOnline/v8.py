...
# --- 步驟 3: 設定專案路徑 ---
PROJECT_PATH = '/content/drive/MyDrive/AvalonServer'
TEMPLATE_FOLDER = os.path.join(PROJECT_PATH, 'v8') # 請確認您的 index.html 在此資料夾

if not os.path.exists(os.path.join(TEMPLATE_FOLDER, 'index.html')):
    print(f"錯誤：找不到檔案 '{os.path.join(TEMPLATE_FOLDER, 'index.html')}'。")
    print("請確認您已在 Google Drive 的 'AvalonServer/v8/' 路徑下放置了 index.html 檔案。")
else:
    print(f"專案路徑設定為: {PROJECT_PATH}")
    print(f"將從 '{TEMPLATE_FOLDER}' 讀取前端範本。")

    # --- 步驟 4: 後端伺服器邏輯 ---
    # ==============================================================================
    # 請在這裡貼上你的 ngrok Authtoken
    # 前往 https://dashboard.ngrok.com/get-started/your-authtoken 取得
    NGROK_AUTH_TOKEN = "30jYpbZvSkVyLCrc15GgGrRkoVf_io8T52Kpb1Nv6uZ5TJYW" # 請替換成您的 Authtoken
    # ==============================================================================

    if NGROK_AUTH_TOKEN:
        conf.get_default().auth_token = NGROK_AUTH_TOKEN
        print("ngrok Authtoken 設定完成。")
    else:
        print("警告：未設定 ngrok Authtoken，連線可能不穩定或失敗。")
    ...
    # 【★★★ 修正點 1 ★★★】
    # 擴充 ALL_ROLES，提供足夠的泛用角色讓房主可以自由配置
    ALL_ROLES = {
        "good": ["梅林", "派西維爾",
    ...
    try:
        # 【★★★ 修正點 ★★★】
        # 直接傳遞埠號變數，不使用關鍵字 'port='
        public_url = ngrok.connect(port)
        print(f"伺服器已啟動，遊戲網址為: {public_url}")
    except Exception as e:
        print(f"Ngrok 連線失敗: {e}")
        # 在 Colab 環境外或 ngrok 失敗時提供本地連結
        print(f"您也可以嘗試在本地瀏覽器訪問: http://127.0.0.1:{port}")
